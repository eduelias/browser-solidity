...
class TxLogger {
    constructor(opts = {}) {
        ...
        this.logBlock = opts.api.editorpanel.registerCommand('block', (args, cmds, append) => {
            var data = args[0]
            var el = renderEmptyBlock(this, data)
            append(el)
        }, { activate: true })
...
function renderEmptyBlock(self, data) {    
    return yo `<span class=${css.txLog}><span class='${css.tx}'>[block:${data.block.number} - ${data.block.transactions.length} transactions - ${data.block.timestamp}]</span></span>`
}
